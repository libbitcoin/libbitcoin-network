...
code channel::notify(messages::identifier id, uint32_t version,
...
void channel::read_heading() NOEXCEPT
{
    BC_ASSERT_MSG(stranded(), "strand");
    ...
    read(heading_buffer_,
        std::bind(&channel::handle_read_heading,
            shared_from_base<channel>(), _1, _2));
}

void channel::handle_read_heading(const code& ec, size_t) NOEXCEPT
{
    BC_ASSERT_MSG(stranded(), "strand");
    ...
    if (head->magic != settings_.identifier)
    {
    ...
    if (head->payload_size > settings_.maximum_payload())
    {
    ...
    read(payload_buffer_,
        std::bind(&channel::handle_read_payload,
            shared_from_base<channel>(), _1, _2, head));
}
...
void channel::handle_read_payload(const code& ec,
    size_t LOG_ONLY(payload_size), const heading_ptr& head) NOEXCEPT
{
    ...
    if (settings_.validate_checksum)
    {
    ...
    if (settings_.minimum_buffer < payload_buffer_.capacity())
    {
        payload_buffer_.resize(settings_.minimum_buffer);
