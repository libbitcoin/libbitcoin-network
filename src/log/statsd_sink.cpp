...
static boost::shared_ptr<text_udp_sink> add_udp_sink(threadpool& pool,
    const asio::ipv6& ip, uint16_t port)
{
    auto socket = boost::make_shared<udp::socket>(pool.service());
    socket->open(udp::v6());

    // Construct a log sink.
    auto endpoint = boost::make_shared<udp::endpoint>(ip, port);
...
void initialize_statsd(threadpool& pool, const asio::ipv6& ip, uint16_t port)
{
    add_udp_sink(pool, ip, port)->set_filter(statsd_filter);
}
