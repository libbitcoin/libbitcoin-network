...
public:
    ...
    bool is_configured(messages::peer::level level) const NOEXCEPT override
    {
...
BOOST_AUTO_TEST_CASE(session__construct__always__expected_settings)
{
    ...
    BOOST_REQUIRE_EQUAL(session.network_settings().threads, expected);
}
