...
BOOST_AUTO_TEST_CASE(distributor_rpc__notify__no_subscriber__success_no_notify)
{
    threadpool pool(2);
    asio::strand strand(pool.service().get_executor());
    distributor_rpc instance(strand);
...
BOOST_AUTO_TEST_CASE(distributor_rpc__subscribe__stopped__returns_stopped_and_invokes_handler)
{
    threadpool pool(2);
    asio::strand strand(pool.service().get_executor());
    distributor_rpc instance(strand);
    ...
    {
        instance.stop(error::service_stopped);
        const auto ec = instance.subscribe(
            [&](const code& ec, method::get_version) NOEXCEPT
            {
                called = true;
                result_ec = ec;
                return false;
    ...
    pool.stop();
    BOOST_REQUIRE(pool.join());
    BOOST_REQUIRE_EQUAL(promise.get_future().get(), error::subscriber_stopped);
    BOOST_REQUIRE(called);
    BOOST_REQUIRE_EQUAL(result_ec, error::subscriber_stopped);
}
...
BOOST_AUTO_TEST_CASE(distributor_rpc__subscribe__multiple__both_return_success_no_invoke)
{
    ...
    boost::asio::post(strand, [&]() NOEXCEPT
    {
        const auto ec1 = instance.subscribe(
            [&](const code&, method::add_element, int, int) NOEXCEPT
            {
                first_called = true;
                return true;
            });
...
BOOST_AUTO_TEST_CASE(distributor_rpc__notify__multiple_subscribers__invokes_both)
{
    ...
    {
        instance.subscribe(
            [&](const code&, method::add_element, int a, int b) NOEXCEPT
            {
                first_called = true;
                first_result_a = a;
                first_result_b = b;
...
BOOST_AUTO_TEST_CASE(distributor_rpc__stop__subscribed_handler__notifies_with_error_code)
{
    ...
    boost::asio::post(strand, [&]() NOEXCEPT
    {
        instance.subscribe([&](const code& ec, method::get_version) NOEXCEPT
        {
            called = true;
            result_ec = ec;
            return false;
...
BOOST_AUTO_TEST_CASE(distributor_rpc__notify__get_version_empty_array_params__success_and_notifies)
{
    ...
    boost::asio::post(strand, [&]() NOEXCEPT
    {
        instance.subscribe([&](const code&, method::get_version) NOEXCEPT
        {
            called = true;
            return true;
        });
...
BOOST_AUTO_TEST_CASE(distributor_rpc__notify__add_element_invalid_type__not_found_no_notify)
{
    ...
    boost::asio::post(strand, [&]() NOEXCEPT
    {
        instance.subscribe([&](const code&, method::add_element, int, int) NOEXCEPT
        {
            called = true;
            return true;
        });
