...
BOOST_AUTO_TEST_CASE(get_compact_transactions__properties__always__expected)
{
    ...
    constexpr auto index = rpc::peer_registry::index_of<get_compact_transactions>();
    BOOST_REQUIRE_EQUAL(rpc::peer_registry::commands().at(index), get_compact_transactions::command);
