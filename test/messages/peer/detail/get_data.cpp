...
BOOST_AUTO_TEST_SUITE(p2p_get_data_tests)
...
static get_data make_mixed_inventory() NOEXCEPT
{
    static get_data value
    {
        inventory_items
        {
...
BOOST_AUTO_TEST_CASE(get_data__properties__always__expected)
{
    ...
    constexpr auto index = messages::peer::registry::index_of<get_data>();
    BOOST_REQUIRE_EQUAL(messages::peer::registry::commands().at(index), get_data::command);
...
BOOST_AUTO_TEST_CASE(get_data__select__selectors__expected_items)
{
...
BOOST_AUTO_TEST_CASE(get_data__filter__types__expected_subset)
{
    const auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__to_hashes__specific_type__expected_hashes)
{
    const auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__count__selector__expected_counts)
{
    const auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__count__type_id__expected_counts)
{
    const auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__any__type_id__expected_presence)
{
    const auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__any_transaction__mixed_items__expected)
{
...
BOOST_AUTO_TEST_CASE(get_data__any_block__mixed_items__expected)
{
    auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__any_witness__mixed_items__expected)
{
    auto inv = make_mixed_inventory();
...
BOOST_AUTO_TEST_CASE(get_data__view__specific_type__filtered_view)
{
    const auto inv = make_mixed_inventory();
