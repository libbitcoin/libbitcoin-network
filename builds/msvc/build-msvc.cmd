...
REM ###########################################################################
REM #  Copyright (c) 2014-2026 libbitcoin-network developers (see COPYING).
REM #
REM #         GENERATED SOURCE CODE, DO NOT EDIT EXCEPT EXPERIMENTALLY
REM #
REM ###########################################################################

REM Script managing the build of libbitcoin-network.
REM
REM Script options:
...
:main
    ...
    if not "!BUILD_SKIP_TESTS!" == "yes" (
        call :build_msbuild "libbitcoin-network" "builds\msvc\%proj_version%" "libbitcoin-network-test" "!REMAPPED_PARAMS!"
        if %ERRORLEVEL% neq 0 (
            exit /b %ERRORLEVEL%
        )
    )
...
:help
    call :msg "Script managing the build of libbitcoin-network."
    call :msg ""
